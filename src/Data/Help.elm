...
topics =
    ...
    , { id = SectionOps
      ...
      , title = "セクション"
      , lines =
            [ line "ブロックの長押し" "並べ替え可能な状態にする（色が変わったらそのまま横へドラッグ）"
            , line "右端のドラッグ" "小節数の変更"
    ...
    , { id = ChordSheet
      ...
      , lines =
            ...
            , line "入力するたびに自動反映" "形式が崩れている間は反映を保留する"
            , line "コードをクリック" "再生位置をそこへ移動する"
            ]
      }
    , { id = VoicingNotation
      , tab = NotationTab
    ...
    , { id = Icons
      ...
      , lines =
            ...
            , line "🔁" "ループ"
            , line "📌 再生位置へ" "再生中にスクロールして追従が外れた時だけ、ピアノロールの右下に表示。押すと再生位置までスクロールして追従を再開"
            , line "✦" "コード進行を編集"
            ]
      }
    , { id = Terms
